...
int main(int argc, char const *argv[])
{
    ...
    int shortestindex = getshortest(pro, n);
    ...
    // table
    cout<<"Pro no\tArrival time\t\twaiting time\t\tTAT"<<endl;
    for(int i=0;i<n;i++)
...
int getshortest(p pro[], int n)
{
    int index = -1; // havent found any shortest paths
    for(int i=0;i<n;i++)
    {
        if(pro[i].posn!=-1)
        {
